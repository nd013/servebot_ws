#include "servebot_controller/simple_controller.hpp"
...
SimpleController::SimpleController(const std::string& name)
                                  : Node(name)
{
    declare_parameter("wheel_radius", 0.060);
    ...
    vel_sub_ = create_subscription<geometry_msgs::msg::TwistStamped>("/servebot_controller/cmd_vel", 10, std::bind(&SimpleController::velCallback, this, _1));
